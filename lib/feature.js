...
const replyImage = async(options, event) => {
    try {
        // return event.reply(options.id)
        // const buffer = await getMediaContent(options.id);
        //  const buffer = await event.getMessageContent();
         return event.reply(JSON.stringify(options));
        return event.reply({
            type: 'image',
...
//取得訊息裡的檔案內容
async function getMediaContent(messageId) {
    try {
...
//將檔案轉成base64
async function getMediaContentAsBase64(event) {
    try {
        const buffer = await event.getMessageContent(messageId);
