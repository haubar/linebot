...
const getGemini = async(options, event) => {
    try {
        const response = await rp(options);
        const message = response.candidates[0].content.parts[0].text || response.error.message || '歹勢啦~Gemini無法回你喲';
...
const replyImage = async(options, event) => {
    try {
        const buffer = await getMediaContent(options.id);
        const filename = `${options.id}.jpg`;
        storage.saveMedia(buffer, filename); // 本機留一份備份
        const url = await storage.getPublicUrl(buffer, filename);
        if (!storage.canPreview()) {
            return event.reply(`收到圖片：${filename}\n連結：${url}`);
        }
        return event.reply([
            {
                type: 'image',
        ...
        console.error('replyImage failed:', err);
        return event.reply(`歹勢啦~我不能回你這個格式~\n錯誤訊息：${err.message}`);
    }
}

const replyAudio = async(options, event) => {
    try {
        const buffer = await getMediaContent(options.id);
        const filename = `${options.id}.m4a`;
        storage.saveMedia(buffer, filename); // 本機留一份備份
        const url = await storage.getPublicUrl(buffer, filename);
        if (!storage.canPreview()) {
            return event.reply(`收到語音訊息\n連結：${url}`);
        }
        return event.reply({
            type: 'audio',
            originalContentUrl: url,
        ...
        console.error('replyAudio failed:', err);
        return event.reply(`歹勢啦~我不能回你這個格式~\n錯誤訊息：${err.message}`);
    }
}

const replyVideo = async(options, event) => {
    try {
        const buffer = await getMediaContent(options.id);
        const filename = `${options.id}.mp4`;
        storage.saveMedia(buffer, filename); // 本機留一份備份
        const url = await storage.getPublicUrl(buffer, filename);
        if (!storage.canPreview()) {
            return event.reply(`收到影片\n連結：${url}`);
        }
        // 注意：previewImageUrl 這裡沿用影片本身的網址，LINE 官方規格要求這欄位要是
        // JPEG/PNG 縮圖，不是影片檔。要正確產生縮圖需要額外做影片截圖（例如 ffmpeg），
        // 目前沒有這個能力，先維持能送出訊息，縮圖顯示可能不正常。
        return event.reply({
            type: 'video',
            originalContentUrl: url,
            previewImageUrl: url
        ...
        console.error('replyVideo failed:', err);
        return event.reply(`歹勢啦~我不能回你這個格式~\n錯誤訊息：${err.message}`);
    }
}

const replyFile = async(options, event) => {
    try {
        const buffer = await getMediaContent(options.id);
        const originalName = options.fileName || `file_${options.id}`;
        const filename = `${options.id}_${originalName}`;
        storage.saveMedia(buffer, filename); // 本機留一份備份
        const url = await storage.getPublicUrl(buffer, filename);
        return event.reply(`收到檔案：${originalName}\n下載連結：${url}`);
    } catch (err) {
        console.error('replyFile failed:', err);
        return event.reply(`歹勢啦~我不能回你這個格式~\n錯誤訊息：${err.message}`);
    }
}

//預設回覆內容
const replyDefault = async(event) => {
    try {
       return event.reply({
            type: 'textV2',
        ...
        console.error('replyDefault failed:', err);
        return event.reply(`歹勢啦~我不能回你這個格式~\n錯誤訊息：${err.message}`);
    }
}
...
//取得訊息裡的檔案內容(從 LINE 下載 buffer)
const getMediaContent = async(messageId) => {
    const buffer = await bot.getMessageContent(messageId);
    if (!buffer || !buffer.length) {
        throw new Error(`取得訊息內容失敗，messageId: ${messageId}`);
    }
...
//另存檔案，暫時不使用
// async function saveMediaContent(event, fileName) {
//     try {
...
//         return filePath;
//     } catch (error) {
//         console.error('Failed to save message content:', error);
//         return null;
//     }
// }
