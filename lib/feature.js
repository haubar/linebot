...
const replyImage = async(options, event) => {
    try {
        ...
        const buffer = await getMediaContentAsBase64(options.id);
...
//取得訊息裡的檔案內容
async function getMediaContent(messageId, event) {
    try {
...
//將檔案轉成base64
async function getMediaContentAsBase64(messageId, event) {
    try {
        const buffer = await event.getMessageContent(messageId);
