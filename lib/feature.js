...
const replyImage = async(options, event) => {
    try {
        ...
        const base64 = await getMediaContentAsBase64(options.id, event);
...
const replyAudio = async(options, event) => {
    try {
        const buffer = await getMediaContent(options.id, event);
        return event.reply({
            type: 'audio',
...
const replyFile = async(options, event) => {
    try {
        const buffer = await getMediaContent(options.id, event);
        // const buffer = await getMediaContentAsBase64(options.id, event);
        return event.reply({
            type: 'file',
            originalContentUrl: buffer,
            fileName: options.fileName || `file_${options.id}`
        });
    } catch (err) {
        return event.reply('歹勢啦~我不能回你這個格式~');
    }
...
//取得訊息裡的檔案內容
const getMediaContent = async(messageId, event) => {
    try {
...
//將檔案轉成base64
const getMediaContentAsBase64 = async(messageId, event) => {
    try {
        // const buffer = await event.getMessageContent(messageId);
        return event.reply(messageId);
        return buffer.toString('base64');
    } catch (error) {
        console.error('Failed to get message content as base64:', error);
        return event.reply("getMediaContentAsBase64  error");
        return null;
    }
}
