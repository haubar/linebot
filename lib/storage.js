...
// storage.to 回傳的是 HTML 分享頁面，不是圖片 binary，沒辦法拿來當 LINE image
// message 的 originalContentUrl；其他 backend 回傳的都是可以直接 fetch 到檔案本體的網址。
const HOTLINK_BACKENDS = new Set(['local', 'catbox', 'r2']);
...
// 目前 backend 回傳的網址能不能直接當圖片來源（hotlink）用來預覽
const canPreview = () => {
    const backendName = process.env.STORAGE_BACKEND || DEFAULT_BACKEND;
    return HOTLINK_BACKENDS.has(backendName);
};
