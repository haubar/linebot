...
class LineBot extends EventEmitter {
	...
	constructor(options) {
		...
		this.dataEndpoint = 'https://api-data.line.me/v2/bot'; // 下載訊息內容(圖片/影片/音檔/檔案)要用這個網域，用 api.line.me 會失敗
	}
	...
	getMessageContent(messageId) {
		return fetch(this.dataEndpoint + '/message/' + messageId + '/content', { method: 'GET', headers: this.headers }).then(function (res) {
			return res.buffer();
		});
	}
	...
	parser() {
		...
		return (req, res) => {
			parser(req, res, () => {
				...
				// 轉發原始 webhook 資料到另一個指定網址（不 await，不影響原本流程）
				if (this.options.forwardUrl) {
					fetch(this.options.forwardUrl, {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
						body: req.rawBody
					}).catch(err => {
						console.error('Forward webhook failed:', err.message);
					});
					// fetch('hhttps://n8n.logflows.com/webhook-test/a0f1c7d2-3b64-4e8a-9c05-7f2ad1e6b431', {
					// 		method: 'POST',
					// 		headers: { 'Content-Type': 'application/json' },
					// 		body: req.rawBody
					// 	}).catch(err => {
			...
			if (req.method === 'POST' && req.url === path) {
				parser(req, res, () => {
					...
					if (this.options.forwardUrl) {
						...
						// fetch('https://n8n.logflows.com/webhook-test/a0f1c7d2-3b64-4e8a-9c05-7f2ad1e6b431', {
						// 	method: 'POST',
						// 	headers: { 'Content-Type': 'application/json' },
						// 	body: req.rawBody
						// }).catch(err => {
				...
				const filename = decodeURIComponent(req.url.slice('/media/'.length));
				const buffer = storage.readMedia(filename);
				if (!buffer) {
					res.statusCode = 404;
					res.setHeader('Content-Type', 'text/html; charset=utf-8');
				...
				res.statusCode = 200;
				res.setHeader('Content-Type', storage.getMimeType(filename));
