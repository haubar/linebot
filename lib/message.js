...
const messageTypeHandlers = {
    ...
    image: (event, msgData) => {
         ...
         try {
            // event.message.content() 返回的是 stream
            const stream = await event.message.content();
            const chunks = [];
            ...
            return new Promise((resolve) => {
                ...
                stream.on('end', () => {
                    ...
                    const fileName = `image_${msgData.id}.jpg`;
                    const filePath = path.join(mediaDir, fileName);
                    fs.writeFileSync(filePath, buffer);
                    
                    const fileUrl = `${process.env.SERVER_URL}/media/${fileName}`;
            ...
            return event.reply('无法获取图片');
        }

        // return feature.replyImage(msgData, event);
        
        // return event.reply(JSON.stringify(msgData, null, 2));
