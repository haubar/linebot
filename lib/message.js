    ...
    'template': async (event) => {
     ...
     'button': async (event) => {
            event.reply({
                ...
                template: {
                    type: 'buttons',
                    thumbnailImageUrl: 'https://example.com/thumbnail.jpg', // 圖片 URL (可選)
                    title: '這是標題', // 標題 (可選)
                    text: '這是訊息內容', // 必填
                    actions: [
                        { type: 'postback', label: '按鈕1', data: 'action=buy&itemid=123' },
                        { type: 'message', label: '按鈕2', text: '按鈕2被點擊' },
                        { type: 'uri', label: '按鈕3', uri: 'https://example.com/' }
                    ]
                }
            }); 
     },
     'carousel': async (event) => {
        event.reply({
            ...
            template: {
                ...
                columns: [
                    {
                        thumbnailImageUrl: 'https://example.com/item1.jpg', // 圖片 URL
                        title: '項目 1', // 標題
                        text: '這是項目 1 的描述',
                        actions: [
                            { type: 'postback', label: '選擇', data: 'action=select&itemid=1' },
                            { type: 'uri', label: '查看', uri: 'https://example.com/item1' }
                        ]
                    },
                    {
                        ...
                        actions: [
                            ...
                            { type: 'uri', label: '查看', uri: 'https://example.com/item2' }
                        ]
                    }
                ]
            }
     ...
     'imagemap': async (event) => {
        event.reply({
            type: 'imagemap',
            baseUrl: 'https://example.com/imagemap', // 圖片 URL (不包含副檔名)
            altText: '這是一個圖文訊息',
            baseSize: { width: 1040, height: 1040 }, // 圖片大小
            actions: [
                {
                    type: 'uri',
                    linkUri: 'https://example.com/link1',
                    ...
                    contents: [
                        {
                            type: 'button',
                            style: 'primary',
                            action: { type: 'uri', label: '查看', uri: 'https://example.com' }
                        }
                    ]
                }
            }
