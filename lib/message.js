...
async function output_message(event) {
    ...
    for (const [command, handler] of Object.entries(command_list)) {
     ...
     //沒有符合的指令就inspire回應
    // return command_list['五十音'](event);
    // return event.reply(`沒這個指令，別做夢了！`);
   return event.reply({
        type: 'textV2',
...
async function output_message(event) {
    ...
    if (handler) {
        return handler(event, event.message);
    } else {
        return event.reply(`暂不支持 ${messageType} 消息类型`);
    }
}
...
const messageTypeHandlers = {
    ...
    image: (event, msgData) => {
        return event.reply({
            type: 'image',
            originalContentUrl: msgData.contentUrl,
            previewImageUrl: msgData.previewImageUrl
        });
    },
