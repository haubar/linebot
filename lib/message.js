...
const messageTypeHandlers = {
    ...
    image: async (event, msgData) => {
        try {
            //   return event.reply(JSON.stringify(event.message));
            // const buffer = await event.message.content();
            const content = await event.message.content();
            
            // 如果是 Response 对象，转换成 buffer
            let buffer;
            if (content && typeof content.buffer === 'function') {
                buffer = await content.buffer();
            } else if (Buffer.isBuffer(content)) {
                buffer = content;
            } else {
                console.log('Content type:', typeof content);
                console.log('Content:', content);
                return event.reply('无法解析图片内容');
            }
            
            const base64 = buffer.toString('base64');
            return event.reply(base64);
            return event.reply({
                type: 'image',
            ...
            return event.reply('dont get image');
        }
    },
