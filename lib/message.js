...
const command_list = {
    'confirm': async (event) => {
     ...
     'imagemap': async (event) => {
        event.reply({
            type: 'imagemap',
            baseUrl: 'https://images.pexels.com/photos/29780009/pexels-photo-29780009/free-photo-of-relaxed-orange-and-white-cat-basking-in-sunlight.jpeg', // 圖片 URL (不包含副檔名)
            altText: '這是一個圖文訊息',
                ...
                body: {
                    ...
                    contents: [
                        ...
                        },
                        {
                            ...
                            margin: 'md',
                            contents: [
                                { type: 'text', text: '描述描述描述描述描述描述描述描述', size: 'sm', color: '#999999' }
                            ]
                        },
                        {
                            type: 'button',
                            style: 'primary',
                            action: { type: 'uri', label: '我是一個按鈕', uri: 'https://www.google.com' }   
                        }
                    ]
                },
                footer: {
                    ...
                    contents: [
                        {
                            ...
                            margin: 'md',
                            contents: [
                                { type: 'text', text: '很多很多很多字', size: 'sm', color: '#999999' }
                            ]
                        }
                        // {
                        //     type: 'button',
                        ...
                        //     action: { type: 'uri', label: '查看', uri: 'https://www.google.com' }   
                        // }
                    ]
                }
            }
    ...
    '過年': async (event) => event.reply("沒有每天在過年的"),
    ...
    '指令': async (event) => {
        const msg = [   
            ...
            '一指：一指斷慾·難返蓬瀛',
            '二指：二指滅道·重墮輪回',
            '三指：三指誅仙·魄形俱喪·萬劫周回生不復',
            '四指：四指滅心·三光同沉·天地暗淡眾生毀',
            '風中捉刀：風中捉刀',
            '壓力大：給你加油打氣',
            '龍破斬：龍破斬咒語',
            'inspire：隨機勵志名言',
    ...
    '誅仙指': async (event) => event.reply(mutiple_message([
        "☝️一指 ✌️二指 🤘三指 🖖四指"  
    ])),
    '一指': async (event) => event.reply(mutiple_message([
        "☝️一指斷慾·難返蓬瀛"  
    ])),
    '二指': async (event) => event.reply(mutiple_message([
        "✌️二指滅道·重墮輪回"  
    ])),
    '三指': async (event) => event.reply(mutiple_message([
        "🤘三指誅仙·魄形俱喪·萬劫周回生不復"  
    ])),
    '四指': async (event) => event.reply(mutiple_message([
        "🖖四指滅心·三光同沉·天地暗淡眾生毀"  
    ])),
    '千少一': async (event) => event.reply(mutiple_message([
        "塵世浮沉近千秋，獨少一念九九九🌹"  
    ])),
    '九九九': async (event) => event.reply(mutiple_message([
        "爭何？多風多雲多月",
        "尋何？至真至善至美",
        "貪何？有權有勢有名",
        "留何？命也樂也長也" 
    ])),
    '999': async (event) => event.reply(mutiple_message([
        "爭何？多風多雲多月",
        "尋何？至真至善至美",
        "貪何？有權有勢有名",
        "留何？命也樂也長也" 
    ])),
    '玫瑰花殺': async (event) => event.reply(mutiple_message([
        "🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹",
    ])),
    '拚': async (event) => event.reply(mutiple_message([
        "現在不拚是要等過年嗎",
    ])),
    '雙十旋龍斬': async (event) => event.reply(mutiple_message([
        "一劍起藝",
        "二劍穿腸紅",
        "五劍剋人",
        "十劍制聖",
    ...
    '七星連線': async (event) => event.reply(mutiple_message([
        "一星起線",
        "二星合線",
        "三星一線",
        "四星直線",
    ...
    '東方孤行': async (event) => event.reply(mutiple_message([
        "利劍一動氣數盡, 日出東方吾孤行"
    ])),
    '金星幻式': async (event) => event.reply(mutiple_message([
        "天地見證 武林絕學~",
        "七星連線"
    ])),
    '笑盡天下': async (event) => event.reply(mutiple_message([
        "哼哼哼~ 笑盡天下"
    ])),
    '任劍誰': async (event) => event.reply(mutiple_message([
        "一生一敗一聲痴",
        "封劍石中再不問",
        "百年難磨今朝恨",
        "再將虎魄換劍魂"
    ...
    '哭': async (event) => event.reply(`ಥ_ಥ`),
    '賭': async (event) => event.reply(`817890587`),
    '財富自由': async (event) => event.reply(mutiple_message([
        "陽春麵加滷蛋",
        "吃布丁不舔蓋",
        "蛋捲不吃屑屑",
        "衛生紙一次抽三張",
...
async function output_message(event) {
    ...
    if (handler) {
        return await handler(event, event.message);
    } else {
        return event.reply({
            type: 'textV2',
            text: '{e1}{e2}{e3}{e4}{e5}{e6}',
            substitution: {
                e1: { type: 'emoji', productId: '5ac21e6c040ab15980c9b444', emojiId: '023' },
                e2: { type: 'emoji', productId: '5ac21e6c040ab15980c9b444', emojiId: '024' },
                e3: { type: 'emoji', productId: '5ac21e6c040ab15980c9b444', emojiId: '025' },
                e4: { type: 'emoji', productId: '5ac21e6c040ab15980c9b444', emojiId: '026' },
                e5: { type: 'emoji', productId: '5ac21e6c040ab15980c9b444', emojiId: '027' },
                e6: { type: 'emoji', productId: '5ac21e6c040ab15980c9b444', emojiId: '028' }
            }
        });
    }
}
...
async function getContent(event) {
    ...
    const buffer = await event.getMessageContent(message_id)
    return buffer;
}
...
const messageTypeHandlers = {
    text: (event, msgData) => {
        ...
        return feature.replyDefault(event);
    },
    
    image: (event, msgData) => {
        return feature.replyDefault(event);
        // return feature.replyImage(msgData, event);
    },
    
    audio: (event, msgData) => {
        return feature.replyAudio(msgData, event);
    },
    
    video: (event, msgData) => {
        return feature.replyVideo(msgData, event);
    },
    
    file: (event, msgData) => {
        return feature.replyFile(msgData, event);
    }
};
...
module.exports = { output_message, getContent };
